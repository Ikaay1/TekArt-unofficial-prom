export const imageUrls = ['first', 'second', 'third', 'fourth'];

export const timeData = [
  {time: 42, day: 'Days'},
  {time: 24, day: 'Hours'},
  {time: 59, day: 'Minutes'},
  {time: 59, day: 'Seconds'},
];

export const nominateData = [
  {image: 'queen', text: 'Prom Queen'},
  {image: 'king', text: 'Prom King'},
  {image: 'superstar', text: 'Prom SuperStar'},
];

export const loveQuotesData = [
  {
    quote:
      'I fell in love with her courage, her sincerity, and her flaming self respect. And it’s these things I’d believe in, even if the whole world indulged in wild suspicions that she wasn’t all she should be. I love her and it is the beginning of everything.',
    author: 'F. Scott Fitzgerald',
  },
  {
    quote:
      'I love you not only for what you are, but for what I am when I am with you. I love you not only for what you have made of yourself, but for what you are making of me. I love you for the part of me that you bring out.',
    author: 'Elizabeth Barrett Browning',
  },
  {
    quote:
      'Thinking of you keeps me awake. Dreaming of you keeps me asleep. Being with you keeps me alive. I love you as certain dark things are to be loved, in secret, between the shadow and the soul.',
    author: 'Bob Charlton',
  },
  {
    quote:
      'I saw that you were perfect, and so I loved you. Then I saw that you were not perfect and I loved you even more.',
    author: 'Angelita Lim',
  },
];

export const ticketsData = [
  {
    price: 'N4K',
    title: 'SINGLE FRECUENTE',
    package: 'Event Access,Item 7,Cocktail',
  },
  {
    price: 'N6.5K',
    title: 'SINGLE CALIDAD',
    package: 'Event Access,Small Chops,Cocktail,Item 7',
  },
  {
    price: 'N7.9K',
    title: 'COUPLE FRECUENTE (2)',
    package: 'Event Access,Item 7,Cocktail',
  },
  {
    price: '13K',
    title: 'COUPLE CALIDAD',
    package: 'Event Access,Small Chops,Item 7,Cocktail,Sovenior',
  },
];

export const loveCalcQuotesData = [
  'He saw that you were perfect, and so he loved you. Then he saw that you were not perfect and he loved you even more',
  'For you both, love is that condition in which the happiness of another person is essential to your own.',
  'For you guys, Love is a promise; it is a souvenir, once given never forgotten. Cheerio!!',
  'For you both, the best thing to hold onto in life is each other.',
  'He needs you like a heart needs a beat.',
  'If she had a flower for every time she thought of you... she could walk through my garden forever.',
  'Take her hand, take her whole life too. For she can’t help falling in love with you.',
  'If you live to be a hundred, he want to live to be a hundred minus one day so he never have to live without you.',
];
